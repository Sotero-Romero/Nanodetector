...
def AnalyseImage(original_image,mean_weight=90,mean_range=11,pore_cut_off=3, Fidelity_Base=50,overlay=True):
...
# Distance transform
    ...
    for gaussian_range in gaussian_ranges:
        ...
        gaussian_filter[bright_distance_map < (300/gaussian_range)] = 200
        gaussian_detection[gaussian_filter == 0] = 0
        new_raw[gaussian_filter==0]=255
        #save_image(new_raw, f'Gaussian Detection, range {gaussian_range}, Fidelity {Fidelity}.png')
        del new_raw
    ...
    filled_detection = cv.dilate(filled_detection, cv.getStructuringElement(cv.MORPH_ELLIPSE, (5, 5)), iterations=1)
    ...
    # Assuming 'rows', 'cols', 'filled_detection', 'pore_cut_off', 'raw_image', 'final_pores', and 'new_directory' are already defined
    for i in range(rows):
        for j in range(cols):
